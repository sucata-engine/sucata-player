...
IS_RELEASED_FUNCTION :: lua_common.LuaFunction {
	...
	func_ptr = proc "c" (L: ^lua.State) -> c.int {
		...
		if !lua_common.validate_string(L, 1, "is_released") do return 0

		button := string(lua.tostring(L, 1))

		slot := -1
		if lua.gettop(L) > 1 {
			if !lua_common.validate_number(L, 2, "is_released") do return 0
			slot = int(lua.tonumber(L, 2))
		}

		value, value_slot := core.gamepad_button_released(button, slot)
